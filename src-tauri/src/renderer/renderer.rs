...
use video_rs::{Time, Locator, EncoderSettings, Encoder};
use wgpu::{BindGroup, BindGroupLayout, RenderPipeline, TextureView};
...
pub struct Renderer {
    ...
    device: wgpu::Device,
    queue: wgpu::Queue,
    window_device: wgpu::Device,
    window_queue: wgpu::Queue,
...
impl Renderer {
    pub async fn new(window: Window, images: &HashMap<u64, DynamicImage>) -> Self {
        ...
        let (window_device, window_queue) = adapter
            .request_device(
        ...
        surface.configure(&window_device, &config);

        let (texture_pipeline, texture_pipeline_bind_groups) =
            texture_pipeline(&device, &queue, surface_format, &images);
    ...
    pub fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>) {
        if new_size.width > 0 && new_size.height > 0 {
            ...
            self.surface.configure(&self.window_device, &self.config);
        }
    }
    ...
    fn render_window(
        ...
        let tex = Texture::from_image_buffer(&self.window_device, &self.window_queue, buffer, None);
        ...
        let window_ratio = self.window_size.width as f32 / self.window_size.height as f32;
        ...
        self.window_queue.submit(iter::once(encoder.finish()));
        output.present();
    ...
    pub async fn render(
        ...
        self.render_window(&self.render_image(frame, images).await)
    }

    pub async fn encode(&mut self, app_data: &mut AppData, app_handle: AppHandle, path: String) {
        let duration = Time::from_nth_of_a_second(app_data.fps);
        let mut position = Time::zero();

        let destination: Locator = PathBuf::from(path).into();
        let settings = EncoderSettings::for_h264_yuv420p(1920, 1080, true);
        let mut encoder = Encoder::new(&destination, settings).unwrap();

        for (frame_index, frame) in app_data.frames.iter().enumerate().into_iter() {
            let frame = self
                .render_image(frame, &app_data.media_resources.images)
                .await;
    ...
    pub fn refresh_texture_pipeline(&mut self, images: &HashMap<u64, DynamicImage>) {
        (self.texture_pipeline, self.texture_pipeline_bind_groups) =
            texture_pipeline(&self.device, &self.queue, self.config.format, images);
    }
